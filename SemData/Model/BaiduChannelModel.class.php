...
class BaiduChannelModel extends CampaignChannelModel
{
    ...
    public function __construct($task_aid)
    {
        ...
        if(!empty($task_aid)) {
            $this->loginInfo = array('B_USERNAME' => $this->account['username'], 'B_PASSWORD' => $this->account['password'], 'B_TOKEN' => $this->account['token']);
        }
    }
    ...
    public function getCampaign($taskid){
        ...
        $allcamp = S($this->channel.$this->account['id'].'_allcamp');
        if(empty($allcamp)){
            $allcamp = $this->getAllCampaign();
            $remain_time = strtotime(date('Y-m-d')." 23:59:59") - time();
            S($this->channel.$this->account['id'].'_allcamp',$allcamp,$remain_time);
        }
    ...
    public function getAdgroup($taskid){
        ...
        if(!empty($campaignIds)){
            ...
            if(empty($adgroup)){
                $adgroup = $this->getAllAdgroupByCampaignIds($campaignIds);
                $remain_time = strtotime(date('Y-m-d')." 23:59:59") - time();
    ...
    public function getKeyword($taskid){
        ...
        $result = $semadgroup->where(array('channel_aid'=>$this->account['id'],'status'=>0))->limit(30)->field('adgroup_id')->select();
        $adgroupIds = array();
        foreach ($result as $value){
        ...
        if(!empty($adgroupIds)){
            ...
            if($exist_keyword!=count($keywords)&&$exist_keyword!=0&&count($keywords)!=0){
                ...
                foreach ($keywords as $key=>$keyword){
                    ...
                    $data['channel'] = $this->channel;
                    $data['channel_aid'] = $this->account['id'];
                    $data['campaign_id'] = $keyword['campaignId'];
    ...
    public function getReport($taskid){
        ...
        if(!empty($schedule)){
            $report_info = $this->getReportInfo($schedule['work_date']);

            if(!empty($report_info)){

                if(is_array($report_info)){

                    $datas_arr = array_chunk($report_info,1000);
                    foreach ($datas_arr as &$datas){
    ...
    public function getReportInfo($date){
        ...
        if(!file_exists($pc_file_name)) {
            ...
            if (empty($task_id)) {
                $task_id = $this->getKeywordReportId($date, 1);
                $remain_time = strtotime(date('Y-m-d') . " 23:59:59") - time();
                S($this->account['id'] . '_reportid_1_' . $date, $task_id, $remain_time);
            }
            sleep(2);
            $task_status = $this->getReportStateByReportId($task_id);
            if ($task_status == '3') {
                $filepath = $this->getReportPath($task_id);
                $csv_content = file_get_contents($filepath);
        ...
        if(!file_exists($wap_file_name)) {
            ...
            if (empty($task_id)) {
                $task_id = $this->getKeywordReportId($date, 2);
                $remain_time = strtotime(date('Y-m-d') . " 23:59:59") - time();
        ...
        if(file_exists($wap_file_name)&&file_exists($wap_file_name)){
            ...
            if($pc_line>1){
                ...
                while (!$pc_csv_object->eof()) {
                    ...
                    $pc_array_str = mb_convert_encoding($content[0], "UTF-8", "GBK");
                    $data = explode("\t",$pc_array_str);
                    if(is_numeric($data[7])){
                        $rs['channel'] = $this->channel;
                        $rs['account_name'] = $data['2'];
                        $rs['campaign_id'] = $data[3];
                        $rs['campaign_name'] = $data[4];
                        //城市
                        $campaign_name_arr = explode('-',$rs['campaign_name']);
                        $rs['city'] = $campaign_name_arr[count($campaign_name_arr)-2];
                        $rs['adgroup_id'] = $data[5];
                        $rs['adgroup_name'] = $data[6];
                        //词性
                        $adgroup_name_arr = explode('-',$rs['adgroup_name']);
                        $rs['property'] = $adgroup_name_arr[count($adgroup_name_arr)-2];
                        if(empty($rs['property'])){
                            $rs['property'] = 'others';
                        }
                        $rs['keyword_id'] = $data[7];
                        $rs['keyword_name'] = $data[9];
                        $rs['views'] = $data[10];
                        $rs['clicks'] = $data[11];
                        $rs['cost'] = $data[12];
                        if($rs['cost']==0){
                            ...
                            $rs['cash'] = $rs['cost']/$this->return_point;
                        }
                        $rs['report_date'] = $date;
                        $rs['device'] = 1;
            ...
            if($wap_line>1){
                ...
                while (!$wap_csv_object->eof()) {
                    ...
                    $wap_array_str = mb_convert_encoding($content[0], "UTF-8", "GBK");
                    $data = explode("\t",$wap_array_str);
                    if(is_numeric($data[7])){
                        $rs['channel'] = $this->channel;
                        $rs['account_name'] = $data['2'];
                        $rs['campaign_id'] = $data[3];
    ...
    public function getOrderReport($taskid){
        ...
        if(!empty($schedule)){
            ...
            if(empty($real_fabu_orders)){
                $real_fabu_orders  = $ordermodel->where(array('urlhistory'=>array('like','%channel=sem&%subchannel=baidu%'),'addtime'=>array('between',array($schedule['work_date']." 00:00:00",$schedule['work_date']." 23:59:59"))))->getField('orderid,dealstatus,urlhistory,shi');
                $fabu_ztc_orders  = $ordermodel->where(array('urlhistory'=>array('like','%channel=sem&%subchannel=baiduztc%'),'addtime'=>array('between',array($schedule['work_date']." 00:00:00",$schedule['work_date']." 23:59:59"))))->field('orderid')->select();
                foreach ($fabu_ztc_orders as $fabu_ztc_order){
                    unset($real_fabu_orders[$fabu_ztc_order['orderid']]);
                }
                $remain_time = strtotime(date('Y-m-d')." 23:59:59") - time();
            ...
            if(empty($real_effect_orders)){
                $real_effect_orders  = $ordermodel->where(array('urlhistory'=>array('like','%channel=sem&%subchannel=baidu%'),'addtime'=>array('between',array($schedule['work_date']." 00:00:00",$schedule['work_date']." 23:59:59"))))->getField('orderid,dealstatus,urlhistory,shi,autoSplitComNum as fd');
                $effect_ztc_orders  = $ordermodel->where(array('urlhistory'=>array('like','%channel=sem&%subchannel=baiduztc%'),'addtime'=>array('between',array($schedule['work_date']." 00:00:00",$schedule['work_date']." 23:59:59"))))->field('orderid')->select();
                foreach ($effect_ztc_orders as $effect_ztc_order){
                    unset($real_effect_orders[$effect_ztc_order['orderid']]);
                }
                $remain_time = strtotime(date('Y-m-d')." 23:59:59") - time();
            ...
            foreach ($real_fabu_orders as $fabu_order){
                ...
                $where['report_date'] = $schedule['work_date'];
                $where['channel'] = $this->channel;
                $where['keyword_id'] = $param_info['tbs_keyword'];
    ...
    public function getCityReport($taskid){
        ...
        if(!empty($schedule)){
            ...
            if(empty($real_effect_orders)){
                $real_effect_orders  = $ordermodel->where(array('urlhistory'=>array('like','%channel=sem&%subchannel=baidu%'),'effecttime'=>array('between',array($schedule['work_date']." 00:00:00",$schedule['work_date']." 23:59:59"))))->getField('orderid,dealstatus,urlhistory,shi,autoSplitComNum as fd');
                $effect_ztc_orders  = $ordermodel->where(array('urlhistory'=>array('like','%channel=sem&%subchannel=baiduztc%'),'effecttime'=>array('between',array($schedule['work_date']." 00:00:00",$schedule['work_date']." 23:59:59"))))->field('orderid')->select();
                foreach ($effect_ztc_orders as $effect_ztc_order){
                    unset($real_effect_orders[$effect_ztc_order['orderid']]);
                }
    ...
    {
        vendor('baiduv4.sms_service_CampaignService');
        $service_cam = new \sms_service_CampaignService($this->loginInfo);
        $campaig = new \GetCampaignRequest();
        $campaig->setCampaignIds(null);
        $fields = array("campaignName","campaignId","pause");
        $campaig->setCampaignFields($fields);
        $campaig->setMobileExtend(1);
        $response = $service_cam->getCampaign($campaig);
     ...
     */
    public function getAllAdgroupByCampaignIds($campaignIds){
        vendor('baiduv4.sms_service_AdgroupService');
        $adgroupService = new \sms_service_AdgroupService($this->loginInfo);
        $adgroup = new \GetAdgroupRequest();
        $idType = 3;
        $fields = array('adgroupId', 'adgroupName', 'pause');
        $adgroup->setAdgroupFields($fields);
        $adgroup->setIdType($idType);

        $arrays = array_chunk($campaignIds,80);
        $result = array();
        foreach ($arrays as &$array){
            $adgroup->setIds($array);
            $response = $adgroupService->getAdgroup($adgroup);
            $response = $response->data;
            $response = object_array($response);
    ...
    {
        vendor('baiduv4.sms_service_KeywordService');
        $idType = 5;
        $getTemp = 0;
        $keywordService = new \sms_service_KeywordService($this->loginInfo);
        $wordRequest = new \GetWordRequest();
        $wordRequest->setIdType($idType);
        $wordRequest->setGetTemp($getTemp);

        $wordRequest->setIds($adgroupIds);
        $fields = array('campaignId', 'adgroupId','keywordId', 'keyword', 'pause');
        $wordRequest->setWordFields($fields);
        $response = $keywordService->getWord($wordRequest);
        $response = $response->data;
        $response = object_array($response);
        return $response;
    }

    public function addkeyword($result){
        $semkeyword = M('SemnKeyword');

        $keywordIds=array();
        foreach ($result as $value){
            array_push($keywordIds,$value);
        }
        ...
        foreach ($response as $keyword){
            $where = array();
            $where['channel_aid'] = $this->account['id'];
            $where['keyword_id'] = $keyword['keyword'];
    ...
    public function getRealAccountReport($startDate,$endDate){
        vendor('baiduv4.sms_service_ReportService');
        $service_cam = new \sms_service_ReportService($this->loginInfo);
        $request=new \GetRealTimeDataRequest();
        $type=new \ReportRequestType();
        $type->setPerformanceData(array("impression","click","cost"));
        $type->setReportType(2);  //关键词报表
        $type->setStartDate($startDate);
        $type->setEndDate($endDate);
        $request->setRealTimeRequestType($type);
        $response=$service_cam->getRealTimeData($request);
        $response = $response->data;
        $response = object_array($response);
        return $response;
    }
     ...
     * @return mixed
     */
    public function getKeywordReportId($date,$is_pc=0){
        vendor('baiduv4.sms_service_ReportService');
        $service_cam = new \sms_service_ReportService($this->loginInfo);
        $request=new \GetProfessionalReportIdRequest();
        $type=new \ReportRequestType();
        $type->setPerformanceData(array("impression","click","cost"));
        $type->setLevelOfDetails(11);  //关键词粒度
        $type->setReportType(14);  //关键词报表
        $type->setDevice($is_pc);  //pc 或者wap端
        $type->setStartDate($date);
        $type->setEndDate($date);
        $request->setReportRequestType($type);
        $response=$service_cam->getProfessionalReportId($request);
        $response = $response->data;
        $response = object_array($response);
     ...
     * @return mixed
     */
    public function getReportStateByReportId($reportId)
    {
        vendor('baiduv4.sms_service_ReportService');
        $service_cam = new \sms_service_ReportService($this->loginInfo);
        $request=new \GetReportStateRequest();
        $request->reportId = $reportId;
        $response=$service_cam->getReportState($request);
        $response = $response->data;
        $response = object_array($response);
        return $response[0]['isGenerated'];
    }

    /**
     * 获取下载路径
     */
    public function getReportPath($reportId)
    {
        vendor('baiduv4.sms_service_ReportService');
        $service_cam = new \sms_service_ReportService($this->loginInfo);
        $request=new \GetReportFileUrlResponse();
        $request->reportId = $reportId;
        $response=$service_cam->getReportFileUrl($request);
        $response = $response->data;
        $response = object_array($response);
        return $response[0]['reportFilePath'];
    }
